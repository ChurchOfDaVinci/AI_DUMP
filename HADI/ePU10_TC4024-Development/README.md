# ePU10
Codebase of TwinCat 4024 software for ePU10 machine

## Software Engineering Manual
The full software engineering manual has been moved to the **[wiki/](wiki/)** directory for better organisation. Each topic is a separate page:

### Guidelines
- [Naming Conventions](wiki/Naming-Conventions.md) — Variable prefix rules for type safety and readability.
- [UI Data Shared Between Cycles](wiki/UI-Data-Shared-Between-Cycles.md) — When cross-cycle mixed state is acceptable and when atomic updates are required.
- [Inter-Object Data Exchange](wiki/Inter-Object-Data-Exchange.md) — How object FBs share data without coupling through get/set structs.

### Architecture Topics
- [FB_PowerControl and FB_GenGridController](wiki/FB-PowerControl-and-FB-GenGridController.md) — Control-architecture split between supervisory mode selection and closed-loop P/Q execution.
- [GenGridController SFC — Mode-Driven Sequencing](wiki/GenGridController-SFC-Mode-Driven-Sequencing.md) — The GenGridSFC action: breaker modes, SFC states, startup sub-sequencer, contactor sync, mode switching, inverter mode selection, and start/stop conditions.
- [MainSFC, ErrorSFC and Object Interaction](wiki/MainSFC-ErrorSFC-and-Object-Interaction.md) — Top-level sequencing, error/shutdown supervisor, the SFCReset mechanism, and how objects are called from the state machine.
- [Diagnostics Interaction](wiki/Diagnostics-Interaction.md) — Centralised diagnostics architecture: ErrorHandling, FB_AddErrorST, eObject_ID offsets, severity levels, and the GetDiagnostics contract.

### Recommendations
- [Identified Inconsistencies and Risks](wiki/Recommendations.md) — Codebase audit findings with prioritised action items.

### Release History
- [Release Notes](wiki/Release-Notes.md) — Version history and changelog.
...
- Moved to Continuous Battery precharging
	I standby mode the main battery would normally charge the UPS and then shutdown to save power.
...
- Change HMI Droop setting behaviour
	...
	This proved to be undesireable. Instead, this setting only controls frequency droop and leaves voltage droop as is (wide by default).
...
- Fixed Coolingsystem emstop bug.
		...
		Even though the rest of the system would be reset and appear healty, the cooling system would prevent operation in the background.
	
- Disable Gridtakeover mode if system is not operation in grid connected mode. 
		The on/off selection would dissappear from the HMI, but the current setting would still effect the system.
...
- 	Input connection CT ratio correction
		The power measurement of the input connection was falsly set to 1200:5A when 1000:5a CT's were used, causing a measurement error.
...
- 400V detection no long looks at 24V psu status
...
### V1.5.0
 ...
 - Cooling system overhauled
  ...
  - Fixed BMS data mapping, data was written to incorect SBMU ID.
...
### V1.4
 ...
 - FB_heatpump: Preheating of heatpump no longer required if the system was operational (powered and preheated, not neccesarily running) in the last  hour
